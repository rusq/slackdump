...
func parseCmdLine(args []string) (params, error) {
	...
	fs.UintVar(&p.appCfg.Options.Tier3Boost, "limiter-boost", slackdump.DefOptions.Tier3Boost, "Tier-3 rate limiter boost in `events` per minute, will be added to the\nbase slack tier event per minute value.")
	fs.UintVar(&p.appCfg.Options.Tier3Boost, "t3boost", slackdump.DefOptions.Tier3Boost, "same as limiter-boost")
	fs.UintVar(&p.appCfg.Options.Tier3Burst, "limiter-burst", slackdump.DefOptions.Tier3Burst, "allow up to `N` burst events per second.  Default value is safe.")
