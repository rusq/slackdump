...
func Test_readEntityList(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			&EntityList{Include: []string{"https://fake.slack.com/archives/CHM82GF99/p1577694990000400"}},
			false,
		},
		{
			"excluded oneliner url",
			args{strings.NewReader("^https://fake.slack.com/archives/CHM82GF99/p1577694990000400"), maxFileEntries},
			&EntityList{Exclude: []string{"https://fake.slack.com/archives/CHM82GF99/p1577694990000400"}},
			false,
		},
		{
			"empty file",
...
func Test_buildEntityIndex(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args{[]string{"123", "234", "^345", "345", "456"}},
			map[string]bool{
				"123": true,
				"234": true,
				"345": false,
		...
		},
		{
			...
			args{[]string{
				...
				"inline_include",
				"^inline_exclude",
				"@" + mkTestFile(td, "override_me\n^excluded\n#comment\nadd_me"),
				"another_inline_include",
				"@" + mkTestFile(td, "second_file_include\n^second_file_exclude"),
			}},
			map[string]bool{
				"override_me":            false,
				"inline_include":         true,
				"inline_exclude":         false,
				"excluded":               false,
				"add_me":                 true,
				"another_inline_include": true,
				"second_file_include":    true,
				"second_file_exclude":    false,
			},
			false,
		},
	}
