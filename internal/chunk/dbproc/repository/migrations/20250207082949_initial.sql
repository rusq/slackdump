-- vim: ft=sql ts=4
-- due to concurrent nature of populating the database, foreign key constraints
-- are only introduced on tables that are guaranteed to be populated in order.
-- +goose Up
...
(
    ...
    LATEST_REPLY TEXT,                             -- LATEST REPLY TS FOR THREAD PARENTS
...
(
    ...
    MESSAGE_ID INTEGER,            -- PARENT MESSAGE ID, EMPTY FOR CHANNEL CANVAS FILES.
    THREAD_ID  INTEGER,            -- POPULATED IF FILE BELONG TO THE THREAD
    IDX        INTEGER   NOT NULL,
    MODE       TEXT      NOT NULL, -- FILE MODE, I.E. 'QUIP', 'HOSTED', 'EXTERNAL'.
    FILENAME   TEXT,
    URL        TEXT,
    DATA       BLOB      NOT NULL,
    PRIMARY KEY (ID, CHUNK_ID),
