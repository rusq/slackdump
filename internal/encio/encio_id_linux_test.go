...
type testFiles struct {
	dockerCgroup    string
	linuxCgroup     string
	newDockerCgroup string
}

// createTestCgroupFiles creates mock files for tests
func createTestCgroupFiles(t *testing.T, dir string) *testFiles {
	// docker cgroup setup
	var files = testFiles{
		dockerCgroup:    filepath.Join(dir, "docker.cgroup"),
		linuxCgroup:     filepath.Join(dir, "linux.cgroup"),
		newDockerCgroup: filepath.Join(dir, "docker_new.cgroup"),
	}

	if err := os.WriteFile(files.dockerCgroup, []byte(dockerCgroup), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(files.linuxCgroup, []byte(linuxCgroup), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(files.newDockerCgroup, []byte(newDockerCgroup), 0644); err != nil {
		t.Fatal(err)
	}
...
func Test_isInContainer(t *testing.T) {
	dir := t.TempDir()
	files := createTestCgroupFiles(t, dir)
	...
	tests := []struct {
		...
		{"docker", args{files.dockerCgroup}, true, false},
		{"linux", args{files.linuxCgroup}, false, false},
		{"docker new", args{files.newDockerCgroup}, true, false},
	}
